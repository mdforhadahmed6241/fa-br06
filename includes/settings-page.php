...
/**
 * =================================================================================
 * 2. ORDER SETTINGS CALLBACKS (Existing)
 * =================================================================================
 */
...
/**
 * =================================================================================
 * 3. COURIER SETTINGS CALLBACKS (NEW)
 * =================================================================================
 */

function br_courier_status_section_callback() {
    echo '<p class="settings-section-description">' . __( 'Define the order statuses that determine the courier-related metrics (Delivered, Returned, Partially Returned).', 'business-report' ) . '</p>';
}

/**
 * Helper function to render a multi-select field for courier statuses.
 *
 * @param string $option_name The name of the option in the database.
 */
function br_courier_status_select_html($option_name) {
    // Default to an empty array for new settings
    ...
    <p class="description">
        <?php _e( 'Hold CTRL (or CMD on Mac) to select multiple statuses.', 'business-report' ); ?>
    </p>
    <?php
    wp_enqueue_script( 'wc-enhanced-select' );
}

function br_courier_delivered_statuses_html() {
    br_courier_status_select_html('br_courier_delivered_statuses');
}

function br_courier_returned_statuses_html() {
    br_courier_status_select_html('br_courier_returned_statuses');
}

function br_courier_partial_returned_statuses_html() {
    br_courier_status_select_html('br_courier_partial_returned_statuses');
}

// Ensure the file ends with a brace if the original file did, which it did.
