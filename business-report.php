...
define( 'BR_PLUGIN_VERSION', '1.6.4' );
...
final class Business_Report {
    ...
    private function force_add_missing_columns() {
        ...
        // --- NEW: Force add courier_status column if it doesn't exist ---
        ...
        // **FIX**: The column must be added if it doesn't exist.
        if (empty($courier_column_exists_result)) {
            error_log("BR Log: Column '{$courier_column_name}' not found in '{$full_table_name}'. Attempting ALTER TABLE.");
            // Add after order_status
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            $alter_result = $wpdb->query("ALTER TABLE `{$full_table_name}` ADD COLUMN `{$courier_column_name}` TINYINT(1) DEFAULT NULL AFTER `order_status`");
            $wpdb->flush(); // Clear cache

            if (false === $alter_result) {
                 error_log("BR Log Error: Failed to add '{$courier_column_name}' column via ALTER TABLE. DB Error: " . $wpdb->last_error);
            } else {
                 error_log("BR Log: Successfully added '{$courier_column_name}' column via ALTER TABLE.");
            }
        } else {
            error_log("BR Log: Column '{$courier_column_name}' already exists in '{$full_table_name}'. Skipping ALTER TABLE.");
        }
    }
